...
const Calculator = () => {
	...
	const LandingDopServicesWindowsCleaning = async () => {
		await axios
			.get<TCategories[]>(`${process.env.REACT_APP_SERVER}/DopCleaningWindows`)
			.then(res => {
	...
	const FCities = async () => {
		await axios
			.get<TCities[]>(`${process.env.REACT_APP_SERVER}/Cities`)
			.then(res => {
					...
					{CurrentServicesSingle !== 'CleaningWindows' && (
						<div className='Calculator--content--BlockPosition--CleaningWinDop'>
							<div className='Calculator--content--BlockPosition--CleaningWinDop--checkbox'>
								...
								/>
								<label htmlFor='CheckboxWindows'>
									Необходимость мойка окон
								</label>
							</div>
